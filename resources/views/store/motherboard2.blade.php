            ...
            <div class="col-12 mt-5">
                <div style="display: none" class="card" id="form">
                    <div class="card-body" id="form">
                        ...
                        @endif
                        <h4 class="bg-info" style="width: 100%;text-align: center;color:azure">Add Motherboard Data</h4>
                        <form action="{{route('motherboard2.store')}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control"  name="motherboard_id" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="">
                    ...
                    <div class="card-body" id="table">
                        <h2 class="bg-primary" style="width: 100%;text-align: center;color:azure">Lab 2 Motherboards Information</h2>
                        ...
                        <div class="data-tables" >
                            <table id="dataTable" class="table  datanew ">
                                ...
                                <tbody>
                                    @foreach($motherboard as $key=>$value)
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td>{{$value->id}}</td>
                                        <td>{{$value->name}}</td>
                                        <td>{{$value->motherboard_id}}</td>
                                        ...
                                        <td>{{$value->model}}</td>
                                        <td>{{$value->details}}</td>
                                        <td><a href="{{route('motherboard2.update',$value->id)}}" class="btn btn-info"><i class="fa fa-edit"></i></a></td>
                                        <td><a href="" class="text-danger"></a>
                                            <form action="{{route('motherboard2.delete',$value->id)}}" method="post" >
                                                @csrf
                                                @method('DELETE')
